...
non_face_scn_path1 = fullfile(data_path, 'train_non_face_scenes'); %We can mine random or hard negatives from here
...
% if exist ('svm_detector_origin3pi_0.mat','file')
%     detector=importdata( 'svm_detector_origin3pi_0.mat');
%     %     w = detector.w;
%     %     b = detector.b;
%     %lambda=detector.lambda;
%     features_pos=detector.features_pos;
%     features_neg=detector.features_neg;
%     sigma=detector.sigma;
%     mu=detector.mu;
%     %features_pos=(features_pos - mu)./sigma;
%     featrues_neg=(features_neg - mu)./sigma;
%     %y1=features_pos*w+b;
%     %y2=features_neg*w+b;
% else
...
% %loop = 8000;
% for loop=1:loops
features_pos_o = get_positive_features( train_path_pos, feature_params );
num_negative_examples = 10000; %Higher will work strictly better, but you should start with 10000 for debugging
features_neg_o = get_random_negative_features( non_face_scn_path, feature_params, num_negative_examples);
%features_neg = get_positive_features( non_face_scn_path, feature_params );
%end
...
%detector_o_path='./svm_detector_origin'
%loop = 8000;
X = vertcat(features_pos_o, features_neg_o);
Y_pos = ones(size(features_pos_o,1),1);

mu = mean(X,1);
sigma = std(X);
X_norm = (X-mu)./sigma;

features_pos=(features_pos_o - mu)./sigma;
features_neg=(features_neg_o - mu)./sigma;
%Y_neg = zeros(size(features_neg,1),1);
Y_neg = ones(size(features_neg_o,1),1)*(-1);
Y = vertcat(Y_pos,Y_neg);
lambda=0.0001;
[w,b] = vl_svmtrain(X_norm', Y, lambda);
%         detector_origin = struct('w',w,'b',b,'lambda',lambda,...
%             'features_pos',features_pos,'features_neg',features_neg, 'mu', mu, 'sigma', sigma, 'X_norm', X_norm);
%         %save('../data/detector/svm_detector_origin', '-struct', 'detector_origin');
%         save(sprintf('../data/detector/svm_detector_origin4pi_loop%s_%s.mat', num2str(loop),num2str(lambda)), '-struct', 'detector_origin');
...
% confidence level.
%     num_loop = 3;
% features_neg_m=features_neg_o;
% % %     for num=1:num_loop
% [~,~,~] = mkdir('../data/neg_mine');
% num=1;
% features_hneg = mine_hard_negatives(non_face_scn_path1, w, b, feature_params, mu, sigma, 1, num);
% features_neg_m = vertcat(features_neg_m,features_hneg);
% Y_neg = ones(size(features_neg_m,1),1)*(-1);
% Y = vertcat(Y_pos,Y_neg);
% %Y=vertcat(Y, Y_neg_m);
% X = vertcat(features_pos_o, features_neg_m);
% mu = mean(X,1);
% sigma = std(X);
% X_norm = (X-mu)./sigma;
% lambda = 0.0001;
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, w, b, feature_params, mu, sigma);
